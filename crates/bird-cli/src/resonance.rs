...
pub async fn refresh_resonance_scores(
    ...
    // Build score map: tweet_id -> (liked, bookmarked, reply_count, quote_count)
    let mut score_map: HashMap<String, (bool, bool, u32, u32)> = HashMap::new();

    // Process likes
    for id in &like_ids {
        let entry = score_map.entry(id.clone()).or_insert((false, false, 0, 0));
        entry.0 = true; // liked
    }
    ...
    let scores: Vec<ResonanceScore> = score_map
        .into_iter()
        .map(|(tweet_id, (liked, bookmarked, reply_count, quote_count))| {
            ResonanceScore::new(
                tweet_id,
                user_id.to_string(),
                liked,
...
pub async fn get_interaction_stats(
    ...
    let replies = storage
        .get_user_reply_tweets(user_id, None)
        .await?
        .len();
    let quotes = storage
        .get_user_quote_tweets(user_id, None)
        .await?
        .len();
